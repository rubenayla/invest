...
from invest.config.loader import load_analysis_config  # noqa: E402
...
class TestEndToEndWorkflows:
    ...
    def mock_stock_universe(self):
        ...
        return {
            ...
            "MSFT": {
                ...
                "trailing_pe": 32.1,
                "price_to_book": 12.5,
                "return_on_equity": 0.361,
                "debt_to_equity": 47.0,
                "current_ratio": 1.9,
                "revenue_growth": 0.180,
                "earnings_growth": 0.250,
                "country": "USA",
                "currency": "USD",
            },
            "GOOGL": {
                ...
                "current_price": 167.06,
                "trailing_pe": 24.8,
                "price_to_book": 5.8,
                "return_on_equity": 0.276,
                "debt_to_equity": 10.5,
                "current_ratio": 2.6,
                "revenue_growth": 0.070,
                "earnings_growth": 0.090,
                "country": "USA",
                "currency": "USD",
            },
            # Japanese Companies
            ...
            "SAP": {
                ...
                "trailing_pe": 18.7,
                "price_to_book": 4.2,
                "return_on_equity": 0.195,
                "debt_to_equity": 65.4,
                "current_ratio": 1.1,
                "revenue_growth": 0.089,
                "earnings_growth": 0.110,
                "country": "Germany",
                "currency": "EUR",
            },
            # Value Stock Example
    ...
    def temp_config_file(self):
        ...
        config_data = {
            ...
            "value": {"max_pe": 30.0, "max_pb": 10.0, "max_ev_ebitda": 20.0},
            "growth": {"min_revenue_growth": 0.03, "min_earnings_growth": 0.02},
            "risk": {"max_beta": 1.8},
            "valuation": {
                "dcf": {
                    "enabled": True,
    ...
    def test_complete_analysis_workflow_us_stocks(self, mock_stock_universe, temp_config_file):
        ...
        with patch("invest.data.yahoo.get_stock_data") as mock_get_data:
            ...
            with patch.object(pipeline, "_get_universe") as mock_universe:
                universe_data = [
                    mock_stock_universe[ticker]
                    for ticker in ["AAPL", "MSFT", "GOOGL", "VALUE_STOCK", "POOR_QUALITY"]
                ]
            ...
            assert value_stock["passes_filters"] is True
            assert poor_stock["passes_filters"] is False
    ...
    def test_international_analysis_workflow(self, mock_stock_universe):
        ...
        intl_config_data = {
            ...
            "risk": {"max_beta": 1.6},
            "valuation": {"dcf": {"enabled": True, "growth_rate": 0.03, "discount_rate": 0.09}},
        }
        ...
        try:
            with patch("invest.data.international.get_market_tickers") as mock_tickers:
                ...
                with patch("invest.data.international.get_international_stock_data") as mock_data:
                    ...
                    with patch.object(pipeline, "_get_universe") as mock_universe:
                        universe_data = [
                            mock_stock_universe["7203.T"],
                            mock_stock_universe["6758.T"],
                        ]
                        mock_universe.return_value = universe_data
                    ...
                    assert toyota["passes_filters"] in [
                        True,
                        False,
                    ]  # May pass/fail based on criteria
    ...
    def test_command_line_interface(self, mock_stock_universe, temp_config_file):
        ...
        with patch("invest.data.yahoo.get_stock_data") as mock_get_data:
            ...
            with patch("invest.analysis.pipeline.AnalysisPipeline._get_universe") as mock_universe:
                universe_data = [
                    mock_stock_universe[ticker] for ticker in ["AAPL", "MSFT", "VALUE_STOCK"]
                ]
                mock_universe.return_value = universe_data
    ...
    def test_output_format_generation(self, mock_stock_universe, temp_config_file):
        ...
        with patch("invest.data.yahoo.get_stock_data") as mock_get_data:
            ...
            with patch.object(pipeline, "_get_universe") as mock_universe:
                universe_data = [
                    mock_stock_universe[ticker]
                    for ticker in ["AAPL", "VALUE_STOCK", "POOR_QUALITY"]
                ]
                mock_universe.return_value = universe_data
