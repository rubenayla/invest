...
- [x] #6 — Backtesting look-ahead bias eliminated — uses fundamental_history DB (commit c7922f0)
- [x] #13 — Quality scoring now continuous (was binary 0/25/50/75/100)
- [x] #17 — Black-Scholes: removed from pipeline (Merton model is a credit/solvency tool, not equity valuation)
- [x] #22 — Banks no longer double-penalized (sector-aware D/E thresholds)
- [x] #23 — Cyclical adjustments dead code removed
- [x] Consensus: filter fv <= 0, confidence uses avg scores not just model count
- [x] Ratios: dead forwardPE/trailingPE removed, sample variance fix
- [x] Growth DCF: cap ROIC at 100% (not reject), fail on negative FCF
- [x] SQLite connection leak in run_classic_valuations.py — wrapped in try/finally
- [x] ModelRegistry re-instantiated per ticker — moved outside loop
- [x] `win_rate` counts periods not trades — now excludes zero-return periods
- [x] Final-row cash placeholder inflates turnover — flagged as liquidation, excluded
- [x] Beta/information ratio frequency mismatch — resample to daily before alignment
- [x] Partial string matching returns wrong metric — exact match first, partial fallback
- [x] 30s timeout vs retry backoff — cap backoff at 30s, increase timeout to 300s
- [x] `financialCurrency` never stored in DB — added column, fetcher writes it, reader returns it
- [x] Stale `currentPrice` gating — allow save to proceed, log warning
- [x] `price_trend_30d` bar-based — now uses 30 calendar-day window
- [x] JSON financials currency for ADRs — store `financial_statements_currency` in JSON
- [x] JSON/SQLite write inconsistency — SQLite first (source of truth), JSON best-effort
- [x] `universal_fetcher.py` D/E normalization — compute from raw components like data_fetcher
- [x] Missing `price_history` table crash — catch OperationalError, return empty, log warning
- [x] Pipeline ordering — documented phases are independent (no reorder needed)
- [x] GBM backtesting: `FROM snapshots` → `FROM fundamental_history` (table was renamed)
- [x] Survivorship bias: filter universe by price availability per date, auto-liquidate delisted holdings
- [x] Intra-period drawdowns: daily portfolio reconstruction from price_history, new `intra_period_max_drawdown` metric
- [x] Earnings quality: accrual-based scoring `(NI - OCF) / TotalAssets` added to ScoringEngine.score_quality()
- [x] `score_growth` key mismatch: `data.get('income_json')` → `data.get('income')` — CAGR was always missing
- [x] CAGR complex number crash: guard `end_val <= 0` (negative NI raised to fractional power)
...
### Black-Scholes-Merton — REMOVED from pipeline
The Merton structural model treats equity as a call option on firm assets.
It's designed for credit analysis (distance-to-default, default probability),
not equity fair value estimation. Using market-derived EV as asset value input
makes the output circular. Removed from dashboard, consensus, registry, and
run scripts. Model file + tests kept in case we repurpose it as a risk metric
(distance-to-default) in the scoring engine.
