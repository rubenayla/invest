...
from invest.data.yahoo import get_stock_data
...
def build_portfolio(
    ...
    try:
        ...
        portfolio_analysis = {
            ...
            'sector_diversification': _analyze_sector_diversification(portfolio_allocation, stock_data_dict),
            'rebalancing_suggestions': _generate_rebalancing_suggestions(portfolio_allocation, stock_data_dict)
        }
...
def optimize_allocation(
    ...
    try:
        ...
        for ticker in all_tickers:
            ...
            if abs(trade_size) >= optimization_constraints['min_trade_size']:
                ...
                rebalancing_plan['trades_required'].append({
                    ...
                    'priority': _calculate_trade_priority(trade_size, current_weight, target_weight)
                })
        ...
        if rebalancing_plan['total_turnover'] > optimization_constraints['max_turnover']:
            rebalancing_plan['warning'] = f"Total turnover {rebalancing_plan['total_turnover']:.1%} exceeds constraint {optimization_constraints['max_turnover']:.1%}"
        ...
        return {
            'error': str(e),
            'message': "Failed to optimize portfolio allocation"
        }
...
def _optimize_portfolio_allocation(scored_stocks: List[Dict], constraints: Dict, max_positions: int) -> Dict[str, float]:
...
def _analyze_portfolio_risk(allocation: Dict[str, float], stock_data_dict: Dict) -> Dict:
    ...
    risk_analysis = {
        'concentration_risk': 'Low' if len(allocation) >= 10 else 'Medium' if len(allocation) >= 5 else 'High',
        'largest_position': max(allocation.values()) if allocation else 0,
        'top_3_concentration': sum(sorted(allocation.values(), reverse=True)[:3]) if len(allocation) >= 3 else sum(allocation.values())
    }
...
def _analyze_sector_diversification(allocation: Dict[str, float], stock_data_dict: Dict) -> Dict:
    ...
    return {
        ...
        'diversification_score': 'High' if len(sector_allocation) >= 5 else 'Medium' if len(sector_allocation) >= 3 else 'Low'
    }


def _generate_rebalancing_suggestions(allocation: Dict[str, float], stock_data_dict: Dict) -> List[str]:
    ...
    if max(allocation.values()) > 0.20:
        ...
        suggestions.append(f"Consider reducing {largest_position[0]} position ({largest_position[1]:.1%}) - exceeds 20% allocation")
    ...
    for sector, weight in sector_weights.items():
        if weight > 0.35:
            suggestions.append(f"High concentration in {sector} sector ({weight:.1%}) - consider diversification")
...
def _analyze_sector_concentration(portfolio_data: Dict, weights: List[float]) -> Dict:
    ...
    return {
        ...
        'concentration_level': 'High' if max_sector_weight > 0.4 else 'Medium' if max_sector_weight > 0.25 else 'Low'
    }
...
def _analyze_size_concentration(portfolio_data: Dict, weights: List[float]) -> Dict:
    ...
    return {
        ...
        'size_diversification': 'High' if min(large_cap_weight, mid_cap_weight + small_cap_weight) > 0.2 else 'Low'
    }
...
def _analyze_style_concentration(portfolio_data: Dict, weights: List[float]) -> Dict:
    ...
    return {
        ...
        'style_balance': 'Balanced' if abs(value_weight - growth_weight) < 0.3 else 'Value-tilted' if value_weight > growth_weight else 'Growth-tilted'
    }
...
def _analyze_financial_health_risk(portfolio_data: Dict, weights: List[float]) -> Dict:
    ...
    return {
        ...
        'financial_health_score': 'Strong' if high_debt_weight < 0.2 and low_profitability_weight < 0.1 else 'Moderate'
    }
...
def _assess_overall_portfolio_risk(risk_analysis: Dict) -> Dict:
    ...
    return {
        ...
        'recommendation': 'Consider diversification improvements' if overall_risk != 'Low' else 'Well-diversified portfolio'
    }


def _calculate_trade_priority(trade_size: float, current_weight: float, target_weight: float) -> float:
...
def _suggest_turnover_reduction(trades: List[Dict], max_turnover: float) -> List[str]:
    ...
    small_trades = [trade for trade in trades if float(trade['trade_size'].rstrip('%')) / 100 < 0.05]
