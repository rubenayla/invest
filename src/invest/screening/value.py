...
def assess_value(data: Dict, thresholds: ValueThresholds) -> Dict:
    ...
    if thresholds.max_ev_ebitda is not None and ev_ebitda > 0:
        ...
        if ev_ebitda <= thresholds.max_ev_ebitda:
            ...
            results['value_flags'].append(f"EV/EBITDA {ev_ebitda:.1f} above threshold {thresholds.max_ev_ebitda:.1f}")
    ...
    if thresholds.max_ev_ebit is not None and ev_ebit and ev_ebit > 0:
        ...
        if ev_ebit <= thresholds.max_ev_ebit:
            ...
            results['value_flags'].append(f"EV/EBIT {ev_ebit:.1f} above threshold {thresholds.max_ev_ebit:.1f}")
    ...
    if thresholds.max_p_fcf is not None and p_fcf and p_fcf > 0:
        ...
        if p_fcf <= thresholds.max_p_fcf:
            ...
            results['value_flags'].append(f"P/FCF {p_fcf:.1f} above threshold {thresholds.max_p_fcf:.1f}")
...
def apply_value_filters(stocks_data: list[Dict], thresholds: ValueThresholds,
                       min_value_score: float = 50.0) -> list[Dict]:
    ...
    filtered = [
        result for result in value_results
        if result['value_score'] >= min_value_score
    ]
...
def identify_deep_value(stocks_data: list[Dict], thresholds: ValueThresholds) -> list[Dict]:
    ...
    for result in value_results:
        if result['value_score'] >= 80:  # High value score
            ...
            if thresholds.max_pe and metrics['pe_ratio'] > 0 and metrics['pe_ratio'] < thresholds.max_pe * 0.6:
                attractive_ratios += 1
            if thresholds.max_pb and metrics['pb_ratio'] > 0 and metrics['pb_ratio'] < thresholds.max_pb * 0.7:
                attractive_ratios += 1
            if thresholds.max_ev_ebitda and metrics['ev_ebitda'] > 0 and metrics['ev_ebitda'] < thresholds.max_ev_ebitda * 0.7:
                attractive_ratios += 1
