...
def assess_growth(data: Dict, thresholds: GrowthThresholds) -> Dict:
    ...
    if thresholds.min_revenue_growth is not None:
        ...
        if revenue_growth >= thresholds.min_revenue_growth:
            ...
            results['growth_flags'].append(f"Revenue growth {revenue_growth:.1%} below threshold {thresholds.min_revenue_growth:.1%}")
    ...
    if thresholds.min_earnings_growth is not None:
        ...
        if earnings_growth >= thresholds.min_earnings_growth:
            ...
            results['growth_flags'].append(f"Earnings growth {earnings_growth:.1%} below threshold {thresholds.min_earnings_growth:.1%}")
    ...
    if thresholds.min_fcf_growth is not None and fcf_growth:
        ...
        if fcf_growth >= thresholds.min_fcf_growth:
            ...
            results['growth_flags'].append(f"FCF growth {fcf_growth:.1%} below threshold {thresholds.min_fcf_growth:.1%}")
    ...
    if thresholds.min_book_value_growth is not None and book_value_growth:
        ...
        if book_value_growth >= thresholds.min_book_value_growth:
            ...
            results['growth_flags'].append(f"Book value growth {book_value_growth:.1%} below threshold {thresholds.min_book_value_growth:.1%}")
...
def apply_growth_filters(stocks_data: list[Dict], thresholds: GrowthThresholds,
                        min_growth_score: float = 50.0) -> list[Dict]:
    """Filter stocks that meet minimum growth requirements."""
    growth_results = screen_growth(stocks_data, thresholds)
    
    filtered = [
        result for result in growth_results
        if result['growth_score'] >= min_growth_score
    ]
...
def identify_growth_at_reasonable_price(stocks_data: list[Dict], growth_thresholds: GrowthThresholds,
                                      max_pe_to_growth: float = 1.5) -> list[Dict]:
