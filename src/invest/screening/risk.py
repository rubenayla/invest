...
def calculate_financial_risk(data: Dict) -> Dict:
    ...
    return {
        ...
        'debt_risk_level': 'high' if debt_equity > 50 else 'moderate' if debt_equity > 25 else 'low',
        'liquidity_risk_level': 'high' if current_ratio < 1.0 else 'moderate' if current_ratio < 1.5 else 'low'
    }
...
def calculate_market_risk(data: Dict) -> Dict:
    ...
    return {
        ...
        'market_risk_level': 'high' if estimated_beta > 1.5 else 'moderate' if estimated_beta > 1.2 else 'low'
    }
...
def calculate_business_risk(data: Dict) -> Dict:
    ...
    cyclical_sectors = [
        'Energy', 'Materials', 'Industrials', 'Consumer Discretionary', 'Financials'
    ]
    ...
    return {
        ...
        'sector_risk_level': 'high' if sector in cyclical_sectors else 'low' if sector in defensive_sectors else 'moderate',
...
def assess_risk(data: Dict, thresholds: RiskThresholds) -> Dict:
    ...
    if thresholds.max_beta is not None:
        if estimated_beta > thresholds.max_beta:
            results['risk_flags'].append(f"Beta {estimated_beta:.2f} above threshold {thresholds.max_beta:.2f}")
    
    # Liquidity check
    if thresholds.min_liquidity_ratio is not None:
        if current_ratio < thresholds.min_liquidity_ratio:
            results['risk_flags'].append(f"Liquidity ratio {current_ratio:.2f} below threshold {thresholds.min_liquidity_ratio:.2f}")
...
def apply_risk_filters(stocks_data: list[Dict], thresholds: RiskThresholds,
                      max_risk_score: float = 60.0) -> list[Dict]:
    ...
    filtered = [
        ...
        if result['overall_risk_score'] <= max_risk_score
    ]
    
    return filtered


def rank_by_risk(stocks_data: list[Dict], thresholds: RiskThresholds, 
