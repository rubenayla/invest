"""
Gemini tools for stock research and analysis.

These tools enable Gemini to perform web-based research and qualitative analysis
...
def research_stock(
    ...
    """
    Research a specific stock using available data and prompt Gemini for web research.
    ...
    Returns:
        Dict containing available stock data and research framework for Gemini
    """
    ...
    if 'error' not in research_data:
        ...
        research_data['gemini_research_prompts'] = research_data.get('research_prompts', [])
...
def analyze_sector(
    ...
    Returns:
        Dict containing sector analysis framework for Gemini
    """
    sector_data = prepare_sector_analysis(sector, focus_areas, include_stocks)
    
    if 'error' not in sector_data:
        # Add Gemini-specific prompts
        sector_data['gemini_research_prompts'] = sector_data.get('research_prompts', [])
...
def get_recent_news(
    ...
    Returns:
        Dict containing news research framework for Gemini
    """
    news_data = prepare_news_research(ticker, news_types, days_back)
    
    if 'error' not in news_data:
        # Add Gemini-specific prompts  
        news_data['gemini_search_prompts'] = news_data.get('search_prompts', [])
...
def compare_competitive_position(
    ...
    if 'error' not in competitive_data:
        # Add Gemini-specific prompts
        competitive_data['gemini_analysis_prompts'] = competitive_data.get('analysis_prompts', [])
