...
def prepare_stock_research(
    ...
    try:
        ...
        research_framework = {
            ...
            'research_prompts': _generate_research_prompts(ticker, stock_data, research_areas, time_horizon)
        }
...
def prepare_news_research(
    ...
    try:
        ...
        news_framework = {
            ...
            'search_prompts': _generate_news_search_prompts(ticker, stock_data, news_types, days_back)
        }
...
def _generate_news_search_terms(ticker: str, stock_data: Dict, news_types: List[str]) -> List[str]:
    ...
    term_generators = {
        ...
        'analyst_updates': lambda: [f"{ticker} analyst", f"{company_name} price target", f"{ticker} upgrade downgrade"],
        'product_launches': lambda: [f"{company_name} product launch", f"{ticker} new product"],
        'regulatory': lambda: [f"{company_name} FDA", f"{ticker} regulation", f"{company_name} approval"],
        'partnerships': lambda: [f"{company_name} partnership", f"{ticker} acquisition", f"{company_name} deal"]
    }
...
def _generate_competitive_analysis_prompts(primary_ticker: str, competitors: List[str], 
    ...
    prompt_generators = {
        ...
        'growth': lambda: f"- Growth comparison: Which company is growing faster? "
                         f"Compare revenue growth, earnings growth, and expansion strategies.",
        
        'valuation': lambda: f"- Valuation comparison: Compare P/E ratios, P/B ratios, and other valuation metrics. "
