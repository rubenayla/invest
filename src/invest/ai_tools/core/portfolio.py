...
def build_optimized_portfolio(
    ...
    try:
        ...
        portfolio_summary = _build_portfolio_summary(
            portfolio_data, allocations, target_amount
        )
        ...
        return {
            'error': str(e),
            'message': 'Failed to build optimized portfolio'
        }
...
def rebalance_portfolio(
    ...
    try:
        ...
        return {
            ...
            'trades': trades,
            'total_turnover': sum(abs(float(t['trade_amount'].replace('$', '').replace(',', ''))) for t in trades),
...
def screen_etfs(
    ...
    try:
        ...
        return {
            'error': str(e),
            'message': f'Failed to screen {category} ETFs'
        }


def _calculate_allocations(portfolio_data: List[Dict], objective: str, 
    ...
    if objective == "equal_weight":
        ...
        return {
            stock['ticker']: stock['market_cap'] / total_mcap 
        ...
        return {
            ticker: max(constraints['min_position_size'], 
...
def _build_portfolio_summary(portfolio_data: List[Dict], allocations: Dict[str, float], 
    ...
    portfolio_beta = sum(
        allocations.get(stock['ticker'], 0) * stock.get('beta', 1.0) 
        for stock in portfolio_data
    )
...
def _calculate_risk_metrics(portfolio_data: List[Dict]) -> Dict[str, Any]:
    ...
    large_cap_weight = sum(
        stock['weight'] for stock in portfolio_data 
...
def _generate_rebalancing_summary(trades: List[Dict]) -> str:
    ...
    total_turnover = sum(
        abs(float(t['trade_amount'].replace('$', '').replace(',', ''))) 
        for t in trades
    )
...
def _generate_etf_recommendations(etf_results: List[Dict], 
    ...
    qualifying_etfs = [
        etf for etf in etf_results
        if (etf.get('expense_ratio', 1.0) <= criteria.get('max_expense_ratio', 0.20) and
            etf.get('aum', 0) >= criteria.get('min_aum', 1e9))
    ]
