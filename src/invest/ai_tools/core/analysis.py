...
from invest.data.yahoo import get_stock_data, get_financials, get_sp500_sample
...
def _build_comparison_data(stock_data: Dict, comparison_metrics: List[str]) -> Dict[str, Any]:
    ...
    if 'financial_health' in comparison_metrics:
        financial_health = _extract_financial_health(stock_data)
        financial_health['free_cash_flow_b'] = financial_health['free_cash_flow'] / 1e9 if financial_health['free_cash_flow'] else 0
...
def _generate_comparison_insights(comparison_data: Dict) -> List[str]:
    ...
    if 'valuation' in comparison_data.get('metrics_compared', []):
        pe_ratios = {ticker: data.get('valuation', {}).get('pe_ratio_trailing', 0) 
                    for ticker, data in companies.items() if data.get('valuation', {}).get('pe_ratio_trailing', 0) > 0}
    ...
    if 'profitability' in comparison_data.get('metrics_compared', []):
        roe_values = {ticker: data.get('profitability', {}).get('return_on_equity', 0) 
                     for ticker, data in companies.items()}
