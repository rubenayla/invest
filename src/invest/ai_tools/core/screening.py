...
from invest.config.loader import load_analysis_config, list_available_configs
...
def run_systematic_screening(
    ...
    try:
        ...
        return {
            'error': str(e),
            'message': f"Failed to run screening with criteria '{criteria}'"
        }
...
def get_available_configs() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'error': str(e),
            'message': "Failed to load configuration information"
        }
...
def create_custom_screening(
    ...
    try:
        ...
        return {
            'error': str(e),
            'message': f"Failed to create custom screen '{name}'"
        }
...
def _load_screening_config(criteria: str) -> AnalysisConfig:
    ...
    if criteria == "default":
        config_path = Path(__file__).parent.parent.parent.parent.parent / "configs" / "default_analysis.yaml"
    else:
        config_path = Path(__file__).parent.parent.parent.parent.parent / "configs" / f"{criteria}.yaml"
...
def _format_screening_results(results: Dict[str, Any], config: AnalysisConfig) -> Dict[str, Any]:
    ...
    for stock in stocks:
        ...
        pick = {
            ...
            'key_metrics': {
                'pe_ratio': f"{basic_data.get('trailing_pe', 0):.1f}" if basic_data.get('trailing_pe') else 'N/A',
                'pb_ratio': f"{basic_data.get('price_to_book', 0):.2f}" if basic_data.get('price_to_book') else 'N/A',
                'roe': f"{basic_data.get('return_on_equity', 0):.1%}" if basic_data.get('return_on_equity') else 'N/A',
                'debt_equity': f"{basic_data.get('debt_to_equity', 0):.1f}" if basic_data.get('debt_to_equity') else 'N/A'
            },
            'flags': {
