#!/usr/bin/env python3
"""
Generate GBM Lite 3-year stock predictions for stocks with limited history (4-6 quarters).

This script:
1. Loads the trained GBM Lite model
2. Makes predictions on stocks that don't qualify for full GBM
3. Saves results to valuation_results with model_name='gbm_lite_3y'
"""
...
def save_to_database(df: pd.DataFrame, predictions: np.ndarray, confidences: list, db_path: str):
    """Save predictions to valuation_results table with model_name='gbm_lite_3y'."""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()

    # Delete existing GBM Lite predictions
    cursor.execute("DELETE FROM valuation_results WHERE model_name = 'gbm_lite_3y'")
    logger.info(f'Deleted {cursor.rowcount} existing gbm_lite_3y predictions')
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        ...
        # GBM Lite predicts 3-year return (e.g., 0.75 = 75% expected return over 3 years)
...
def main():
    ...
    model_path = project_root / 'neural_network/training/gbm_lite_model_3y.txt'
    ...
    if not model_path.exists():
        ...
        logger.error('  uv run python train_gbm_lite_stock_ranker.py --target-horizon 3y')
        return 1

    # Get stocks needing lite model
