#!/usr/bin/env python3
"""
Generate GBM Opportunistic 3-year stock predictions and save to valuation_results.
...
def save_to_database(df: pd.DataFrame, predictions: np.ndarray, confidences: list, db_path: str):
    ...
    cursor.execute("DELETE FROM valuation_results WHERE model_name = 'gbm_opportunistic_3y'")
    logger.info(f'Deleted {cursor.rowcount} existing gbm_opportunistic_3y predictions')
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        ...
        # GBM predicts opportunistic 3-year return (e.g., 0.75 = 75% expected return over 3 years)
        ...
        # Details JSON
        details = {
            'predicted_return_opportunistic_3y': float(predicted_return),
...
def main():
    ...
    model_path = project_root / 'neural_network/training/gbm_opportunistic_model_3y.txt'
