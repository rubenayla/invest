...
def add_price_features(df: pd.DataFrame, conn) -> pd.DataFrame:
    ...
    def calc_price_features(row):
        ...
        if snapshot_id not in price_groups.groups:
            return pd.Series({feat: 0.0 for feat in PRICE_FEATURES})
...
def save_to_database(df: pd.DataFrame, predictions: np.ndarray, confidences: list, db_path: str):
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        ...
        try:
            ...
            current_price = stock_data['info'].get('currentPrice') or 0
        except:
            logger.warning(f'Could not get current price for {ticker}')
            current_price = 0

        if not current_price or current_price == 0:
            logger.warning(f'Skipping {ticker}: no valid current price')
            continue
