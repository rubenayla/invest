#!/usr/bin/env python3
"""
Train gradient-boosted tree models with strict holdout evaluation.

This script trains on data up to a cutoff date (default: 2023-12-31) and
evaluates on data after that date (2024-2025) as a true holdout test.

This allows rigorous verification of model performance without any
information leakage from the test period.

Key differences from train_gbm_stock_ranker.py:
- Hard cutoff date for training/test split
- Reports both CV validation IC AND true holdout IC
- Saves models with '_holdout' suffix
- Designed to verify that high ICs are genuine, not overfitting artifacts
"""
...
class GBMStockRankerHoldout:
    """Gradient-boosted tree stock ranker with strict holdout evaluation."""
    ...
    def __init__(
        ...
        """
        Initialize GBM stock ranker with holdout evaluation.
        ...
        train_cutoff : str
            Cutoff date for training data (YYYY-MM-DD)
        """
        ...
        logger.info(f'Initialized {model_type} ranker for {target_horizon} horizon (HOLDOUT mode)')
    ...
    def prepare_training_data(
        ...
        """
        Prepare training and holdout test datasets with standardization.
        ...
        -------
        Tuple[pd.DataFrame, pd.DataFrame, List[str], List[str]]
            (train_df, holdout_df, numeric_features, categorical_features)
        """
        ...
        # Split into train (before cutoff) and holdout (after cutoff)
        ...
        # Winsorize numeric features (cross-sectional, per date) - TRAIN ONLY
        ...
        # Standardize numeric features (z-score per date) - TRAIN ONLY
        logger.info('Standardizing features (z-score per date)')
        train_df = standardize_by_date(train_df, numeric_features)

        # Apply same preprocessing to holdout (using its own statistics)
        if len(holdout_df) > 0:
    ...
    def train(
        ...
        """
        Train LightGBM model with purged CV on training data only.

        Parameters
        ----------
        df : pd.DataFrame
            Prepared training data (before cutoff)
        numeric_features : List[str]
    ...
    def evaluate(
        ...
        categorical_features : List[str]
            Categorical feature names
        label : str
            Label for logging (e.g., 'CV Validation' or 'Holdout Test')
    ...
    def save_model(self, path: str | None = None):
        """Save trained model."""
        if path is None:
            path = f'../gbm_model_{self.target_horizon}_holdout.txt'
...
def main():
    parser = argparse.ArgumentParser(description='Train GBM stock ranker with holdout evaluation')
    ...
    logger.info('=' * 60)
    logger.info(f'HOLDOUT EVALUATION COMPLETE - {args.target_horizon} horizon')
    logger.info('=' * 60)
    ...
    results_path = Path(__file__).parent / f'holdout_results_{args.target_horizon}.json'
